...
const GEMINI_API_KEY = "COLLE_TA_CLE_ICI"; // <-- COLLE TA CLE ICI
...
async function runBaobabAI(query) {
  ...
  if(currentSecurity === 'strong' || GEMINI_API_KEY === "COLLE_TA_CLE_ICI") {
    aiBlock.classList.add('hidden');
    return;
  }

  aiBlock.classList.remove('hidden');
  $('#aiText').innerText = `Baobab IA réfléchit...`;
  $('#aiBtn').classList.add('hidden');
  $('#aiSources').innerHTML = '';

  try {
    const lang = currentLang === 'en-US'? 'anglais' : 'français';
    const prompt = `Tu es Baobab IA, l'assistant de Baobab Search. Réponds à cette question en 4 lignes maximum, en ${lang}. Sois direct et factuel. Question: ${query}`;
    ...
    $('#aiSources').innerHTML = `<span>Source: Gemini 2.0 Flash</span>`;
  } catch(e) {
    $('#aiText').innerText = "Erreur de connexion à Baobab IA. Vérifie ta clé API.";
  }
}
